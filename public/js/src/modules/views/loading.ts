import * as utils from 'modules/lib/utils';

interface ILoadingViewConfig {
  loadingClassName: string;
  loadingHTMLContent: string;
  indicatorType: 'indeterminate' | 'progress' | string;
  indicatorColor: string;
}

interface ILoadingViewState {
  hasLoaded: boolean;
  progress: number;
  autoIncrementTimeoutId?: number;
}

export class LoadingView {
  $loadingView: JQuery<HTMLElement>;
  $loadingIcon: JQuery<HTMLElement>;
  $loadingBar: JQuery<HTMLElement>;
  $indicator: JQuery<HTMLElement>;

  config: ILoadingViewConfig;
  state: ILoadingViewState = { hasLoaded: false, progress: 0 };

  constructor(options?: Partial<ILoadingViewConfig>) {
    this.config = utils.defaults(options, {
      loadingClassName: 'wp-spa-loading-view--loading',
      loadingHTMLContent: require('raw-loader!./../views/html/loading-view.html').default,
      indicatorType: 'indeterminate',
      indicatorColor: ''
    });

    this.$loadingView = $(this.config.loadingHTMLContent);
    this.$loadingIcon = this.$loadingView.find('.wp-spa-loading-view__icon');
    this.$loadingBar = this.$loadingView.find('.wp-spa-loading-view__progress-bar');
    this.$loadingView.addClass('wp-spa-loading-view--' + this.config.indicatorType);
  }


  show(amount: number) {
    this.$loadingView.addClass(this.config.loadingClassName);
    if (this.config.indicatorType == 'progress' && (amount === 0 || amount > 0)) this.setLoadingProgress(amount);
  }

  reset() {
    this.state.progress = 0;
    this.state.hasLoaded = false;
    this.$loadingView.removeClass(this.config.loadingClassName);
    this.$loadingBar.css({
      'opacity': ''
    });
  }

  setLoadingProgress(amount: number) {
    if (!this.state.progress) {
      this.state.progress = 0;
      this.$loadingBar.css({ 'transform': 'translate3d(0, 0, 0)' });
    }

    // use amount only if higher than current progress
    this.state.progress = amount && amount > this.state.progress ? amount : this.state.progress;
    switch (amount) {
      case 100:
        clearTimeout(this.state.autoIncrementTimeoutId);
        this.state.hasLoaded = true;
        this.$loadingBar.css({
          'transform': 'translate3d(100%, 0, 0)',
          'opacity': '0'
        });
        setTimeout(() => this.reset(), 1000);
        break;

      case 0:
      default:
        this.$loadingBar.css({
          'transform': 'translate3d(' + this.state.progress + '%, 0, 0)'
        });
        this.state.autoIncrementTimeoutId = window.setTimeout(() => {
          if (this.state.progress < 75 && !this.state.hasLoaded) this.show(this.state.progress + 2)
        }, 500);
        break;
    }

  }

  hide() {
    if (this.config.indicatorType == 'progress') {
      this.setLoadingProgress(100);
      return;
    }

    this.$loadingView.find('.wp-spa-loading-view__icon').css({ opacity: 0 });

    window.setTimeout(() => {
      // hide everything
      this.$loadingView.find('.wp-spa-loading-view__icon').css({ opacity: '' });
      this.$loadingView.removeClass(this.config.loadingClassName);
    }, 650);
  }

  appendTo($parent: JQuery<HTMLElement>) {
    this.$indicator = this.config.indicatorType == 'progress' ? this.$loadingBar : this.$loadingIcon;
    this.$indicator.css({
      'background-color': this.config.indicatorColor
    });
    $parent.append(this.$loadingView);
  }
}
